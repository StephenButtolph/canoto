...
func Canoto(inputFilePath string) error {
	...
	return writeCanoto(outputFile, packageName, messages)
}

func writeCanoto(w io.Writer, packageName string, messages []message) error {
...
func makeSize(m message) string {
	const (
		...
		size += len(canoto__${escapedStructName}__${escapedFieldName}__tag) + canoto.Size${suffix}
	}
`
...
`
	...
	return writeMessage(m, messageTemplate{
		...
		size += len(canoto__${escapedStructName}__${escapedFieldName}__tag) + canoto.Size${suffix}(c.${fieldName})
	}
`,
