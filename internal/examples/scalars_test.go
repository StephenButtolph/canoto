...
func BenchmarkScalars_MarshalCanoto(b *testing.B) {
	benchmarks := []struct {
		...
		{
			name: "full",
			make: makeFullScalars,
		},
		{
			name: "primitives",
			make: makeSimpleScalars,
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name+" stack", func(b *testing.B) {
			for range b.N {
				s := bm.make()
				s.MarshalCanoto()
			}
		})
		b.Run(bm.name+" heap", func(b *testing.B) {
			s := bm.make()
			for range b.N {
				s.MarshalCanoto()
			}
...
func BenchmarkScalars_UnmarshalCanoto(b *testing.B) {
	full := makeFullScalars()
	simple := makeSimpleScalars()
	...
	for _, bm := range benchmarks {
		for _, unsafe := range []bool{false, true} {
			b.Run(bm.name+"/unsafe="+strconv.FormatBool(unsafe), func(b *testing.B) {
				for range b.N {
					var (
						s      Scalars
...
func BenchmarkScalars_MarshalProto(b *testing.B) {
	benchmarks := []struct {
		name string
		make func() pb.Scalars
	}{
		{
			name: "full",
			make: makeFullPBScalars,
		},
		{
			name: "primitives",
			make: makeSimplePBScalars,
		},
	}
	for _, bm := range benchmarks {
		b.Run(bm.name+" stack", func(b *testing.B) {
...
func BenchmarkScalars_UnmarshalProto(b *testing.B) {
	full := makeFullPBScalars()
	fullBytes, err := proto.Marshal(&full)
	require.NoError(b, err)

	simple := makeSimplePBScalars()
...
func makeSimpleScalars() Scalars {
	return Scalars{
		Int8:     31,
		Int16:    2164,
		Int32:    216457,
...
func makeFullPBScalars() pb.Scalars {
	return pb.Scalars{
		Int8:     31,
		Int16:    2164,
		Int32:    216457,
...
func makeSimplePBScalars() pb.Scalars {
	return pb.Scalars{
		Int8:     31,
		Int16:    2164,
		Int32:    216457,
